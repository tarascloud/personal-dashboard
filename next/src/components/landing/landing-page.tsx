...
// DES-20260512-0005: shared CTA button classes so hero buttons all have the
// same height, radius, font, and padding regardless of icon/text length.
// Use these on every primary/secondary CTA in the hero and Open Source sections.
const CTA_BASE =
  "inline-flex items-center justify-center gap-2 rounded-[6px] h-12 px-8 text-base font-semibold transition-all whitespace-nowrap";
const CTA_PRIMARY =
  `${CTA_BASE} bg-[var(--landing-accent)] text-[var(--landing-accent-dark)] shadow-lg shadow-[var(--landing-glow)] hover:bg-[var(--landing-accent-bright)] hover:shadow-xl hover:shadow-[var(--landing-glow-strong)]`;
const CTA_SECONDARY =
  `${CTA_BASE} border border-[var(--landing-border)] text-[var(--landing-text)] hover:bg-[var(--landing-surface)] hover:border-[var(--landing-border-hover)]`;
...
export function LandingPage({ freeSpots, githubEnabled }: { freeSpots: FreeSpotsData; githubEnabled: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--landing-bg)] text-[var(--landing-text)] font-[Inter]">
      ...
      <nav className="sticky top-0 z-50 border-b border-[var(--landing-border)] bg-[var(--landing-bg)]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {/* DES-20260512-0002: collapse full wordmark below 390px to prevent
                overlap with lang switcher + actions. "PD" shows on narrow
                viewports, full name from sm: up. */}
            <span className="text-sm sm:text-lg font-bold text-[var(--landing-accent)] truncate">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <a
              href="/login"
              className="hidden sm:inline text-sm font-medium text-[var(--landing-text-muted)] hover:text-[var(--landing-text)] transition-colors"
            >
              {t("sign_in")}
            </a>
      ...
      {/* DES-20260512-0004: WCAG 2.4.1 main landmark + skip target.
          Root layout already provides the "Skip to content" anchor pointing
          to #main-content; landing page must expose that target. */}
      ...
      <section className="relative overflow-hidden px-4 pt-16 pb-20 sm:px-6 sm:pt-24 sm:pb-28">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          {/* DES-20260512-0006: scarcity badge — pulse dot + icon + bordered
              pill so visitors read it as a genuine availability indicator,
              not flavor text. Red+pulse when ≤3 spots left, amber otherwise. */}
                ...
                <span className="relative flex h-2.5 w-2.5">
                  <span
                    className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${
                      freeSpots.remaining <= 3 ? "bg-red-500" : "bg-amber-400"
                    }`}
                  />
                  <span
          ...
          {freeSpots.remaining === 0 && (
            <div className="mt-6 flex justify-center">
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--landing-border)] bg-[var(--landing-surface)] px-4 py-1.5 text-sm text-[var(--landing-text-muted)]">
                <span className="inline-flex h-2.5 w-2.5 rounded-full bg-[var(--landing-text-faint)]" />
                {t("spots_full")}
              </div>
            </div>
          )}
      ...
      <section className="px-4 py-16 sm:px-6 sm:py-20 bg-[var(--landing-bg-alt)]">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`${CTA_SECONDARY} group bg-[var(--landing-surface)] hover:bg-[var(--landing-surface-hover)]`}
          >
            <Github className="h-5 w-5" />
            {t("open_source_cta")}
          </a>
